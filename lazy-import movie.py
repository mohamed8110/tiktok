...
st.set_page_config(page_title="MNWS TikTok — iPad ready", layout="centered")
...
def render_video(title, desc, *, bg_bytes=None, logo_bytes=None, music_bytes=None,
    ...
    try:
        ...
        st.error("MoviePy is niet geïnstalleerd. Controleer je requirements en herstart.")
        raise
...
bg_up   = st.file_uploader("Achtergrond (jpg/png) — optioneel", type=["jpg","jpeg","png"])
logo_up = st.file_uploader("Logo (PNG)", type=["png"])
music_up= st.file_uploader("Muziek (mp3) — optioneel", type=["mp3"])

st.subheader("🎥 Eén clip")
